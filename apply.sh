...
git am -3 $CATHEDRA_PATH/dcos/patches/0001-core-jni-Switch-to-O3.patch
cd $BASE_BUILD_DIR

cd build/soong
echo "[i] Applying 0002-soong-clang-builds-with-O3.patch"
git am -3 $CATHEDRA_PATH/dcos/patches/0002-soong-clang-builds-with-O3.patch
cd $BASE_BUILD_DIR

cd bionic
echo "[i] Applying 0003-libc-switch-to-jemalloc-from-scudo.patch"
git am -3 $CATHEDRA_PATH/dcos/patches/0003-libc-switch-to-jemalloc-from-scudo.patch
cd $BASE_BUILD_DIR

...
echo "[i] Fetching Rust 1.65.0 build for the ROM build..."
rm -rf prebuilts/rust/linux-x86/1.65.0
mkdir -p prebuilts/rust/linux-x86/1.65.0
cd prebuilts/rust/linux-x86/1.65.0
curl https://ftp.travitia.xyz/clang/rust-1-65-0.tar.xz | tar -xJ
cd $BASE_BUILD_DIR

# Temporary picks until they get merged upstream
repopick -p 13572                    # Fix search bar spacing
repopick -p 12848                    # Themed icon setting in launcher

$CATHEDRA_PATH/apply.py $CATHEDRA_PATH/patches
$CATHEDRA_PATH/apply.py $CATHEDRA_PATH/patches/rust-1.65
